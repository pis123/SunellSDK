...
*/
	...
	typedef int					int_t;		//????,?????????Å£
	typedef unsigned int		uint_t;		//?????????,?????????Å£
	typedef long				long_t;		//????,?????????Å£
	typedef unsigned long		ulong_t;	//????,?????????Å£
	...
	typedef long long			int_t;		//????,?????????Å£
	typedef unsigned long long	uint_t;		//?????????,?????????Å£
	typedef long long			long_t;		//????,?????????Å£
	typedef unsigned long long	ulong_t;	//????,?????????Å£
#elif defined(__RJ_APPLE__)
	#if __LP64__
	typedef unsigned short		RJ_BOOL;
	typedef signed char			int8;
	typedef unsigned char		uint8;
